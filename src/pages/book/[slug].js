...
const Book = () => {
    ...
    const bookevent = async () => {
        ...
        toast.info("Event is being booked...", toastconfig);
        ...
        try {
            ...
            formData.append("ticketNumber", ticketCount);
            ...
            const response = await axios.post("/api/user/bookevent", formData, {
                headers: headers,
            });
            if (response.status === 200 && response.data.success === true) {
                setTicketCount(1)
                toast.success("Booking successful!", toastconfig);
                ...
                toast.error("Booking failed. Please try again later.", toastconfig);
                clearCanvas()
            }
        } catch (error) {
            console.error("error");
            toast.error("An error occurred while booking. Please try again later.", toastconfig);
            clearCanvas()
        }
    };
    ...
    return (
        <>
            ...
            <div className="bg-gray-700 h-full w-full">
                <div className="font-extrabold text-xl text-center p-5 md:mb-16">Confirm Booking</div>
                <div className="md:flex md:justify-center">
                    ...
                    <div className="sm:w-2/3 sm:pl-8 sm:py-8 sm:border-l md:ml-5 border-gray-200 sm:border-t-0 border-t mt-4 pt-4 sm:mt-0 text-center sm:text-left">
                        ...
                        <p className="leading-relaxed text-lg mb-4 text-white">Please take photo of your face in proper lighting, This Photo will be matched at the entry gate!</p>
                        <input
                            type="number"
                            className="border border-gray-300 bg-gray-600 text-white px-3 py-2 mb-3 rounded-lg focus:outline-none focus:ring focus:border-blue-300"
                        ...
                        />
                        <div className=''>
                            <button disabled={!isphotoverified} onClick={bookevent} className="inline-flex text-white bg-indigo-600 disabled:bg-indigo-100 items-center border border-indigo-800 p-2 rounded-md ">Confirm Booking
